...
**`llm_enrichment.py` optionally adds a short, grounded "Why this matters" paragraph** to each new article via the Claude API (`generate_editorial_context()`, called from `generate_mr_informer_article()`). It requires `ANTHROPIC_API_KEY` (local `.env` + GitHub Actions secret) and the `anthropic` package (`requirements.txt`); if either is missing, or the call errors/refuses, it silently returns `None` and the article publishes exactly as before — this must never block or fail the pipeline. The model is instructed to ground the paragraph strictly in the given headline/excerpt and never invent new facts, consistent with the no-fabrication rule above. Existing articles are not retroactively enriched — only new ones going forward.
...
Local `.env` (gitignored) holds `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `SITE_DOMAIN`, and optionally `ANTHROPIC_API_KEY` (for `llm_enrichment.py` — see above; the pipeline works fine without it). The **service role key bypasses RLS entirely** and is only ever used by the Python scripts — never reference it from `supabase_client.js` or any browser-shipped file. GitHub Actions needs its own copies as repo secrets (`SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, optionally `ANTHROPIC_API_KEY`) and a repo variable (`SITE_DOMAIN`).
